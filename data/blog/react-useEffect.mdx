---
title: 'Day 10 — What is useEffect?'
date: '2025-11-10'
lastmod: '2025-11-10'
tags: ['reactjs']
summary: 'Understanding useEffect - the essential hook for managing side effects in React function components, from data fetching to event listeners, with proper cleanup and dependency management.'
---
...
1. **setState is called** → React schedules a re-render
2. **Render phase** → React runs the component to produce Virtual DOM
3. **Commit phase** → React updates the real DOM
4. **useLayoutEffect** runs synchronously (before paint)
5. **Browser paints** → user sees the updated UI
...
2. **Empty array `[]`** → runs only once, after the first render
3. **Specific dependencies `[value]`** → runs whenever those values change
...
1. Doing API calls directly in render  → should be in `useEffect`
2. Forgetting cleanup  → memory leaks
3. Wrong dependency array  → stale data or infinit re-renders
4. Unconditional `setState` inside effect  → infinie loop
